...
public class SpikeGroundAttack : BossAttack
{
    ...
    public float spikeOffsetY = -3f;
    public GameObject warningPrefab;
    public float warningOffsetY = -1.7f;
    ...
    private IEnumerator AttackCoroutine()
    {
        ...
        for (int iteration = 0; iteration < numberOfAttacks; iteration++)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Instantiate(warningPrefab, new Vector3(playerX + (i * distanceBetweenSpikes), startPosY + warningOffsetY, 0f), Quaternion.identity);
            }
            for (int i = -1; i > -10; i--)
            {
                Instantiate(warningPrefab, new Vector3(playerX + (i * distanceBetweenSpikes), startPosY + warningOffsetY, 0f), Quaternion.identity);
            ...
            for (int i = 0; i < 10; i++)
            {
                Instantiate(spikePrefab, new Vector3(playerX + (i * distanceBetweenSpikes), startPosY + spikeOffsetY, 0f), Quaternion.identity);
            }
            for (int i = -1; i > -10; i--)
            {
                Instantiate(spikePrefab, new Vector3(playerX + (i * distanceBetweenSpikes), startPosY + spikeOffsetY, 0f), Quaternion.identity);
